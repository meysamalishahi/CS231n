...
def content_loss(content_weight, content_current, content_original):
    ...
    scalar_content_loss = content_weight * ((content_current - content_original)**2).sum()
...
def gram_matrix(features, normalize=True):
    ...
    gram = features.mm(features.t()).view(N, C, C)
    if normalize:
        gram /= H * W * C
...
def style_loss(feats, style_layers, style_targets, style_weights):
    ...
    for i in range(n):
        ...
        gram_current_image_i = gram_matrix(feats[style_layers[i]])
        loss += style_weights[i] * ((gram_current_image_i - style_targets[i])**2).sum()
...
def tv_loss(img, tv_weight):
    ...
    V_loss = torch.sum((img[:, :, :, 1:] - img[:, :, :, :-1])**2)
    tv_loss = tv_weight * (H_loss + V_loss)
