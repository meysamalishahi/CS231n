...
def sample_noise(batch_size, dim, seed=None):
    ...
    return 2 * torch.rand(batch_size, dim) - 1.0
...
def build_dc_classifier(batch_size):
    ...
    model = nn.Sequential(
            ...
            nn.Conv2d(in_channels=1 , out_channels=32 , kernel_size=5), # [batch_size, 32, 28, 28]
            nn.LeakyReLU(negative_slope=0.01),
            nn.MaxPool2d(kernel_size=2, stride=2), # [batch_size, 32, 14, 14]
        
            nn.Conv2d(in_channels=32 , out_channels=64 , kernel_size=5), # [batch_size, 64, 8, 8]
            nn.LeakyReLU(negative_slope=0.01),
...
def build_dc_generator(noise_dim=NOISE_DIM):
    ...
    model = nn.Sequential(
        ...
        Unflatten(-1, 128, 7, 7),
        nn.ConvTranspose2d(128, 64, 4, stride=2, padding=1), # (batch_size, 64, 14, 14)
        nn.ReLU(),
        nn.BatchNorm2d(64),
        nn.ConvTranspose2d(64, 1, 4, stride=2, padding=1), #(batch_size, 1, 28, 28)
        nn.Tanh(),
        Flatten() #(batch_size, 1*28*28)
    )
